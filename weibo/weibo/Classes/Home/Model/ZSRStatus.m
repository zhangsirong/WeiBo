...
- (NSAttributedString *)attributedTextWithText:(NSString *)text
{
    ...
    [text enumerateStringsMatchedByRegex:pattern usingBlock:^(NSInteger captureCount, NSString *const __unsafe_unretained *capturedStrings, const NSRange *capturedRanges, volatile BOOL *const stop) {
        ...
        ZSRTextPart *part = [[ZSRTextPart alloc] init];
        part.special = YES;
    ...
    // 排序
    // 系统是按照从小 -> 大的顺序排列对象
    [parts sortUsingComparator:^NSComparisonResult(ZSRTextPart *part1, ZSRTextPart *part2) {
    ...
    for (ZSRTextPart *part in parts) {
        ...
        if (part.isEmotion) { // 表情
            ...
            NSString *name = [ZSREmotionTool emotionWithChs:part.text].png;
            if (name) { // 能找到对应的图片
...
- (void)setRetweeted_status:(ZSRStatus *)retweeted_status
{
